...
def load_cnn_model(md_pth):
    use_cuda = torch.cuda.is_available()
    device = torch.device("cuda" if use_cuda else "cpu")
    model = CNN3D(5).to(device)
    ckp_path = (md_pth)
    # "/storage/gxt/cnn_model_save/New_Epoch-100-Test_loss-0.9707233309745789.pth"
    model = load_ckp(ckp_path, model, device)
    model.eval()

    return model, device

def cnn_score(recon_roi, md_pth):
    dataloader = cnn_data_pre(recon_roi)
    model, device = load_cnn_model(md_pth)
    pred_sum = 0
    total = 0
