import os
os.environ['XLA_PYTHON_CLIENT_PREALLOCATE'] = 'false' #disable the pre-allocation
import random
import tempfile
import warnings
...
def get_opt_views(ct_model, reference_object, num_selected_views, apply_noise = False, r_1=0.002, r_2=0.5, prev_selected_angles=np.array([]), verbose=0, seed=None,roi=None):
    """
    ...
    Args:
        ...
        prev_select_angle (ndarray, optional): Previously selected view angles. Defaults to an empty NumPy array.
    ...
    if not np.isin(prev_selected_angles, angle_candidates).all():
    ...
    with tempfile.TemporaryDirectory() as data_store_dir:
        # Compute recon bases
        gamma = compute_view_basis_functions(ct_model, reference_object, r_1=r_1, data_store_dir=data_store_dir,apply_noise = apply_noise, seed=seed,roi=roi)
    ...
    optimal_angle_inds, vcl_value = compute_opt_angle_subset(R, gamma, angle_candidates, num_selected_views, r_2, prev_selected_angles, seed=seed)

    return optimal_angle_inds, vcl_value



def compute_view_basis_functions(ct_model, ref_object, r_1, data_store_dir, apply_noise,seed=None,roi=None):
    """
    ...
    print(f'simulated projection shape is {ref_sino.shape}')
    ...
    if roi is None:
        ...
        sparse_ref_object = jnp.asarray(ref_object[roi])
        norm_sparse_ref = jnp.linalg.norm(sparse_ref_object)
        sparse_ref_object /= (norm_sparse_ref + eps)
    ...
    angles_total = ct_model.get_params('angles')
    ...
    for i in tqdm.trange(num_views, desc='Computing and storing view basis functions'):
        ...
        if roi is None:
            ...
            new_angles = angles_total[i:i + 1]
            ct_model_temp = copy_ct_model(ct_model, new_angles)
            recon_i = ct_model_temp.back_project(view_sino)
            recon_i = jnp.asarray(recon_i[roi])
        ...
        if roi is None:
            ...
            gamma[i, 0] = jnp.sum(recon_i * sparse_ref_object)
...
def compute_opt_angle_subset(R, gamma, candidate_angles, K, r_2, prev_selected_angles=np.array([]), search_min=30, max_iterations = 100, seed=None):
    """
    ...
    Args:
        ...
        prev_select_angle (ndarray): Previously selected view angles.
    ...
    prev_angle_inds = np.where(np.isin(candidate_angles, prev_selected_angles))[0]
