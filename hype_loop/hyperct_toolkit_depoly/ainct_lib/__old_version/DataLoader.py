...
def OBLoader(ob_paths, paras, wav_st, wav_end, out_path, save = True): # data_path, ob_str
    ...
    if os.path.exists(ob_fname):
        ...
        ob = get_ob(ob_base_folders, paras, clean_path, wav_st, wav_end, level = False)
        if save:
            dxchange.write_tiff(ob, dtype=np.float32, fname = ob_fname, overwrite=True)
            print('saving OB to {}'.format(ob_fname))
        return ob

def ProjLoader(data_path, ob_paths, proj_str, paras, cali_paras, wav_st, wav_end, out_path, level = False, save = True):
    ...
    proj_paths = gen_paths(proj_base_folders) if level else proj_base_folders
    angle_list = info_get(proj_paths, -1, 'Angle') # -1 for no tpx1 detectore/ -2 for tpx3
